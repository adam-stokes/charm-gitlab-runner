...
# If layer options are used, add this to gitlabrunner
# and import layer in lib_gitlab_runner
@pytest.fixture
def mock_layers(monkeypatch):
    ...
    monkeypatch.setattr('lib_gitlab_runner.layer.options', options)
...
def mock_hookenv_config(monkeypatch):
    ...
    monkeypatch.setattr('lib_gitlab_runner.hookenv.config', mock_config)


@pytest.fixture
def mock_remote_unit(monkeypatch):
    monkeypatch.setattr('lib_gitlab_runner.hookenv.remote_unit', lambda: 'unit-mock/0')


@pytest.fixture
def mock_charm_dir(monkeypatch):
    monkeypatch.setattr('lib_gitlab_runner.hookenv.charm_dir', lambda: '/mock/charm/dir')
...
def gitlabrunner(tmpdir, mock_hookenv_config, mock_charm_dir, monkeypatch):
    ...
    monkeypatch.setattr('lib_gitlab_runner.GitlabrunnerHelper', lambda: helper)
